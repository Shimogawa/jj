...
inline uint64_t _jj_hash_func(const void* item, uint64_t seed0,
                              uint64_t seed1) {
    const jj_jsonobj* j = (const jj_jsonobj*)item;
    return hashmap_sip(j->name, strlen(j->name), seed0, seed1);
}

inline int _jj_comp_func(const void* a, const void* b, void* udata) {
...
inline _jj_lexstate* _jj_new_lexstate(const char* original,
...
inline bool _jj_lexstate_isatend(_jj_lexstate* s) {
    return s->cur_idx >= s->length;
}

inline bool _jj_lexstate_bufequals(_jj_lexstate* s, const char* other,
                                   size_t cnt) {
    return s->buflen == cnt && strncmp(s->strbuf, other, cnt);
}
